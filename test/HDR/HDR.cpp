...
VkShaderModule HDRshaderModule; //
VkPipeline HDRpipeline; //
VkDescriptorSet HDRdescriptorSet;
VkImage HDRrenderTargetTextureImage;
VkDeviceMemory HDRrenderTargetTextureMemory;
VkImageView HDRrenderTargetTextureView;
...
void recordCommandBuffers()
{
	...
	for (uint32_t i = 0; i < swapChainImages.size(); i++) {
		...
		{ //render to screen
			...
			vkCmdBindDescriptorSets(presentCommandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, samplePipelineLayout, 0, 1, &HDRdescriptorSet, 0, 0);
...
void CreateShaders()
{
	...
	char sample_fs_asm_code[] =
			...
			"sig_none ; tlb_color_all = or.always(r0, r0) ; nop = nop(r0, r0) ;" //Write to HDR RT
...
void CreatePipeline()
{
	...
	{ //create sample pipeline
		...
		shaderStageCreateInfo[0].module = HDRshaderModule;
		shaderStageCreateInfo[1].module = HDRshaderModule;
		pipelineInfo.renderPass = HDRrenderPass;
		res = vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, NULL, &HDRpipeline);
	}
...
void CreateDescriptorSet()
{
	...
	vkAllocateDescriptorSets(device, &allocInfo, &HDRdescriptorSet);
	...
	{ //create HDR descriptor set
		VkDescriptorImageInfo imageInfo;
		imageInfo.imageView = HDRrenderTargetTextureView;
		imageInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		imageInfo.sampler = textureSampler;

		VkWriteDescriptorSet writeDescriptorSet = {};
		writeDescriptorSet.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
		writeDescriptorSet.dstSet = HDRdescriptorSet;
		writeDescriptorSet.dstBinding = 0;
		writeDescriptorSet.descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
		writeDescriptorSet.pImageInfo = &imageInfo;
		writeDescriptorSet.descriptorCount = 1;
...
void CreateHDRrenderTarget()
{
	...
	vkCreateImage(device, &imageCreateInfo, 0, &HDRrenderTargetTextureImage);

	VkMemoryRequirements mr;
	vkGetImageMemoryRequirements(device, HDRrenderTargetTextureImage, &mr);
	...
	vkAllocateMemory(device, &mai, 0, &HDRrenderTargetTextureMemory);

	vkBindImageMemory(device, HDRrenderTargetTextureImage, HDRrenderTargetTextureMemory, 0);
	...
	view.subresourceRange.layerCount = 1;
	view.subresourceRange.levelCount = 1;
	view.image = HDRrenderTargetTextureImage;
	vkCreateImageView(device, &view, 0, &HDRrenderTargetTextureView);
	...
	vkCreateRenderPass(device, &renderPassInfo, 0, &HDRrenderPass);

	VkImageView attachments = HDRrenderTargetTextureView;

	VkFramebufferCreateInfo framebufferCreateInfo = {};
	framebufferCreateInfo.renderPass = HDRrenderPass;
	...
	vkCreateFramebuffer(device, &framebufferCreateInfo, 0, &HDRframeBuffer);
}
