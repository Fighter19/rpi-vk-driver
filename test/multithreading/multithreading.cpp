...
std::vector<threadData> threadDataVector;
...
void createCommandQueues() {
	...
	if (vkAllocateCommandBuffers(device, &allocInfo, &primaryCommandBuffer) != VK_SUCCESS) {
		std::cerr << "failed to allocate presentation command buffers" << std::endl;
		assert(0);
	}

	threadDataVector.resize(numThreads);
	...
	for(uint32_t c = 0; c < numThreads; ++c)
	{
		...
		if (vkCreateCommandPool(device, &poolCreateInfo, nullptr, &threadDataVector[c].commandPool) != VK_SUCCESS) {
			std::cerr << "failed to create command queue for presentation queue family" << std::endl;
			assert(0);
		}
		...
		VkCommandBufferAllocateInfo allocInfo = {};
		allocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
		allocInfo.commandPool = threadDataVector[c].commandPool;
		allocInfo.level = VK_COMMAND_BUFFER_LEVEL_SECONDARY;
		allocInfo.commandBufferCount = 1;

		if (vkAllocateCommandBuffers(device, &allocInfo, &threadDataVector[c].commandBuffer) != VK_SUCCESS) {
			std::cerr << "failed to allocate presentation command buffers" << std::endl;
			assert(0);
		}

		threadDataVector[c].numVertices = (vertices.size() / 2) / numThreads;
		threadDataVector[c].vertexOffset = c * threadDataVector[c].numVertices;
	}
...
void threadFunc(uint32_t threadIdx, VkCommandBufferInheritanceInfo inheritanceInfo)
{
	...
	vkBeginCommandBuffer(threadDataVector[threadIdx].commandBuffer, &beginInfo);

	vkCmdBindPipeline(threadDataVector[threadIdx].commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline);

	VkDeviceSize offsets = 0;
	vkCmdBindVertexBuffers(threadDataVector[threadIdx].commandBuffer, 0, 1, &vertexBuffer, &offsets );
...
void recordCommandBuffers()
{
	...
	inheritanceInfo.framebuffer = fbs[imageIndex];
	...
	for(uint32_t c = 0; c < numThreads; ++c)
	{
		threads.push_back(std::thread(threadFunc, c, inheritanceInfo));
	}
...
void CreateVertexBuffer()
{
	...
	for(float y = -1.0; y <= 1.0; y += stepH)
	{
		for(float x = -1.0; x <= 1.0; x += stepW)
		{
			float p = float(rand()) / RAND_MAX;
			if(p < 0.5)
			{
				vertices.push_back(x);
				vertices.push_back(y + stepH);

				vertices.push_back(x + stepW);
				vertices.push_back(y + stepH);

				vertices.push_back(x + stepW * 0.5);
				vertices.push_back(y);
			}
		}
	}

	fprintf(stderr, "Num Triangles: %u\n", vertices.size()/6);
	fprintf(stderr, "Num Vertices: %u\n", vertices.size()/2);
	...
	{ //create staging buffer
		...
		ci.size = sizeof(float) * vertices.size();
