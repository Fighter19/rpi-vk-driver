...
void recordCommandBuffers()
{
	...
	for (uint32_t i = 0; i < swapChainImages.size(); i++) {
		...
		VkDeviceSize offsets[2] = {0, sizeof(float)*2*3};
		VkBuffer buffers[2] = {vertexBuffer, vertexBuffer};
		vkCmdBindVertexBuffers(presentCommandBuffers[i], 0, 2, buffers, offsets);
...
0x500009e7009e7000 sig_unlock_score nop nop, r0, r0 ; nop nop, r0, r0
	...
	char fs_asm_code[] =
			...
			"sig_none ; nop = nop.pm(r0, r0, ra3, nop) ; r0.8d = v8min.always(a, a) ;"
			"sig_none ; tlb_color_all = or.always(r0, r0) ; nop = nop(r0, r0) ;"
			"sig_end ; nop = nop(r0, r0) ; nop = nop(r0, r0) ;"
			"sig_none ; nop = nop(r0, r0) ; nop = nop(r0, r0) ;"
			"sig_unlock_score ; nop = nop(r0, r0) ; nop = nop(r0, r0) ;"
...
void CreateVertexBuffer()
{
	unsigned vboSize = sizeof(float) * (2 * 3 + 3); //3 x vec2 + 3
