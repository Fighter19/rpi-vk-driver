...
uint16_t getBigEndian(uint16_t num)
{
	return ((num & 0x00ff) << 8) | ((num & 0xff00) >> 8);
}

char* readPKM(const char* fileName)
{
	uint32_t pkm_magic;
	((uint8_t*)&pkm_magic)[0] = 'P';
	((uint8_t*)&pkm_magic)[1] = 'K';
	((uint8_t*)&pkm_magic)[2] = 'M';
	((uint8_t*)&pkm_magic)[3] = ' ';
	...
	uint32_t magic_number = ((uint32_t*)buf)[0];
	if(magic_number != pkm_magic)
	{
		fprintf(stderr, "PKM magic number not found: %u\n", magic_number);
		return 0;
	}

	uint16_t version = ((uint16_t*)buf)[2];
	char versionStr[] = { 0, 0, 0 };
	memcpy(versionStr, &version, 2);
	printf("PKM Version: %s\n", versionStr);

	if(versionStr[0] != '2' && versionStr[1] != '0')
	{
	...
	uint16_t dataType = getBigEndian(((uint16_t*)buf)[3]);
	printf("PKM data type: %s (%u)\n", dataTypeStr[dataType], dataType);

	uint16_t extendedWidth = getBigEndian(((uint16_t*)buf)[4]);
	uint16_t extendedHeight = getBigEndian(((uint16_t*)buf)[5]);
	uint16_t originalWidth = getBigEndian(((uint16_t*)buf)[6]);
	uint16_t originalHeight = getBigEndian(((uint16_t*)buf)[7]);

	printf("Extended Image size: %i x %i\n", extendedWidth, extendedHeight);
	printf("Original Image size: %i x %i\n", originalWidth, originalHeight);

	char* imageBuf = buf + 8 * 2;

	//image is subdivided into 4x4 blocks, each block is 64bits
	uint32_t bytesNeeded = (extendedWidth >> 2) * (extendedHeight >> 2) * 8;

	char* retBuf = (char*)malloc(bytesNeeded);
	memcpy(retBuf, imageBuf, bytesNeeded);
